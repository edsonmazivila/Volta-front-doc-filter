...
export async function fetchWithGracefulFallback<T>(
  ...
  const { logErrors = true, errorContext = "fetchWithGracefulFallback" } =
    options;
...
export function getPathsForEntity(entity: keyof typeof CacheTags): string[] {
  const entityToPaths: Record<string, string[]> = {
    ...
    TIMESHEETS: [
      CachePaths.TIMESHEETS,
      CachePaths.MY_TIMESHEETS,
      CachePaths.PAYROLL,
      CachePaths.DASHBOARD,
    ],
    LEAVES: [CachePaths.LEAVES, CachePaths.DASHBOARD],
    DOCUMENTS: [
      CachePaths.DOCUMENTS,
      CachePaths.MY_DOCUMENTS,
      CachePaths.DASHBOARD,
    ],
    PAYROLL_RUNS: [
      CachePaths.PAYROLL,
