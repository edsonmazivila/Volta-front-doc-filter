...
export const getAttendanceRecords = cache(async (filters?: AttendanceFilters): Promise<AttendanceRecord[]> => {
...
export const getPendingJustifications = cache(async (): Promise<AttendanceJustification[]> => {
  return fetchWithGracefulFallback(
    async () => {
      const authHeader = await getAuthCookieHeader()

      const res = await fetch(`${API_BASE_URL}/api/attendance/justifications/pending`, {
        headers: {
          'Content-Type': 'application/json',
          ...(authHeader && { Cookie: authHeader }),
        },
        next: { tags: [CacheTags.ATTENDANCE_JUSTIFICATIONS], revalidate: 30 },
      })
...
export const getMyAttendance = cache(async (month?: string): Promise<AttendanceRecord[]> => {
  return fetchWithGracefulFallback(
    async () => {
      const authHeader = await getAuthCookieHeader()
...
export async function createAttendanceAction(prevState: unknown, formData: FormData): Promise<ActionResult> {
  ...
  try {
    ...
    if (!res.ok) {
      ...
      return { errors: { _form: [error.message || 'Failed to create attendance'] } }
    }
...
export async function updateAttendanceAction(prevState: unknown, id: number, formData: FormData): Promise<ActionResult> {
  const cookieHeader = await getAuthCookieHeader()
...
export async function deleteAttendanceAction(id: number): Promise<ActionResult> {
  const cookieHeader = await getAuthCookieHeader()
  ...
  try {
    ...
    if (!res.ok) {
      const error = await res.json()
      return { errors: { _form: [error.message || 'Failed to delete attendance'] } }
    }
...
export async function approveJustificationAction(justificationId: number): Promise<ActionResult> {
  const cookieHeader = await getAuthCookieHeader()

  try {
    const res = await fetch(`${API_BASE_URL}/api/attendance/justifications/${justificationId}/approve`, {
      method: 'POST',
      headers: {
        ...(cookieHeader && { Cookie: cookieHeader }),
      },
    ...
    if (!res.ok) {
      const error = await res.json()
      return { errors: { _form: [error.message || 'Failed to approve justification'] } }
    }
...
export async function rejectJustificationAction(justificationId: number): Promise<ActionResult> {
  const cookieHeader = await getAuthCookieHeader()

  try {
    const res = await fetch(`${API_BASE_URL}/api/attendance/justifications/${justificationId}/reject`, {
      method: 'POST',
      headers: {
        ...(cookieHeader && { Cookie: cookieHeader }),
      },
    ...
    if (!res.ok) {
      const error = await res.json()
      return { errors: { _form: [error.message || 'Failed to reject justification'] } }
    }
...
export async function exportAttendanceCSV(month: string): Promise<Blob | null> {
  ...
  try {
    const res = await fetch(`${API_BASE_URL}/api/attendance/export?month=${month}`, {
      headers: {
        ...(cookieHeader && { Cookie: cookieHeader }),
      },
    })
