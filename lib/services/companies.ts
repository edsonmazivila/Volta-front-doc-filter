...
export async function getAllCompanies(): Promise<{
  ...
  const response = await fetch(
    `${API_BASE_URL}/api/companies?include_stats=true`,
    {
      method: "GET",
      headers,
      credentials: "include",
...
export async function getCompaniesByOrganization(
  ...
  const response = await fetch(
    `${API_BASE_URL}/api/organizations/${organization_id}/companies`,
    {
      method: "GET",
      headers,
      credentials: "include",
...
export async function getCompanyById(id: string): Promise<Company> {
  ...
  if (!response.ok) {
    const error = await response
      .json()
      .catch(() => ({ message: "Failed to fetch company" }));
...
export async function createCompany(data: CreateCompanyData): Promise<Company> {
  ...
  if (!response.ok) {
    const error = await response
      .json()
      .catch(() => ({ message: "Failed to create company" }));
...
export async function updateCompany(
  ...
  if (!response.ok) {
    const error = await response
      .json()
      .catch(() => ({ message: "Failed to update company" }));
...
export async function deactivateCompany(id: string): Promise<void> {
  ...
  if (!response.ok) {
    const error = await response
      .json()
      .catch(() => ({ message: "Failed to deactivate company" }));
...
export async function getCompanyStats(companyId: string): Promise<{
  ...
  const response = await fetch(
    `${API_BASE_URL}/api/companies/${companyId}/stats`,
    {
      method: "GET",
      headers,
      credentials: "include",
  ...
  if (!response.ok) {
    const error = await response
      .json()
      .catch(() => ({ message: "Failed to fetch company stats" }));
