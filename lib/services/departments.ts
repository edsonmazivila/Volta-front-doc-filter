      ...
      return raw.map(
        (
          ...
          parent_department_name: d.parent_department_name
            ? String(d.parent_department_name)
            : null,
...
export const getDepartmentById = cache(
  async (id: string): Promise<DepartmentDetail | null> => {
    return fetchWithGracefulFallback(
      async () => {
        const cookieHeader = await getAuthCookieHeader();
        ...
        return {
          ...
          parent_department_name: data.parent_department_name
            ? String(data.parent_department_name)
            : null,
          manager_id: data.manager_id ? String(data.manager_id) : null,
          manager: data.manager
            ? {
                id: String(data.manager.id || ""),
                full_name: String(data.manager.full_name || ""),
                email: String(data.manager.email || ""),
              }
            : null,
          is_active: Boolean(data.is_active),
          created_at: String(data.created_at || ""),
          updated_at: String(data.updated_at || ""),
          child_departments: childDepartments.map(
    ...
    async () => {
      ...
      if (!res.ok) {
        ...
        return {
          ...
          departmentsWithManager: departments.filter((d) => d.manager_id)
            .length,
        };
      }
      ...
      return {
        ...
        activeDepartments:
          Number(data.activeDepartments ?? data.active ?? 0) || 0,
        inactiveDepartments:
          Number(data.inactiveDepartments ?? data.inactive ?? 0) || 0,
        departmentsWithManager:
          Number(data.departmentsWithManager ?? data.with_manager ?? 0) || 0,
      };
    },
    {
      totalDepartments: 0,
  ...
  try {
    ...
    if (!res.ok) {
      ...
      return {
        errors: {
          _form: [
            error.message || error.error || "Failed to create department",
          ],
        },
      };
    }
...
export async function updateDepartmentAction(
  ...
  const isActive = hasIsActiveField
    ? isActiveValues.some((v) => v === "true" || v === "on")
    : undefined;

  // Handle parent_department_id - empty string means remove parent (set to null)
  const parentDeptValue = formData.get("parent_department_id");
  const parentDepartmentId =
    parentDeptValue === "" ? null : parentDeptValue || undefined;
  ...
  try {
    ...
    if (!res.ok) {
      const error = await res.json().catch(() => ({}));
      return {
        errors: { _form: [error.message || "Failed to update department"] },
      };
    }
