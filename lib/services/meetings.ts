...
export const getMyMeetings = cache(async (params?: { status?: string }): Promise<Meeting[]> => {
  try {
    const qs = new URLSearchParams()
    if (params?.status) qs.append('status', params.status)

    const cookieHeader = await getAuthCookieHeader()
    const res = await fetch(`${API_BASE_URL}/api/meetings/my${qs.toString() ? `?${qs}` : ''}`, {
      headers: {
...
export const getAvailableParticipants = cache(async (): Promise<Array<{ id: string; name: string; email?: string }>> => {
  try {
    const cookieHeader = await getAuthCookieHeader()
...
export async function createMeetingAction(
  ...
  try {
    // Parse participants
    const participantIds = formData.getAll('participant_ids').filter(Boolean) as string[]
...
export async function respondToMeetingAction(
  ...
  try {
    ...
    if (!res.ok) {
      ...
      return {
        errors: {
          _form: [errorData.error || `Failed to respond to meeting: ${res.status}`],
        },
      }
    }
