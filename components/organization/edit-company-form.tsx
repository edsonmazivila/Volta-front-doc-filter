...
import type { UpdateCompanyData } from "@/lib/services/companies";
...
export function EditCompanyForm({ company }: EditCompanyFormProps) {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const result = await updateCompanyAction(company.id, payload);
      
      if (!result.success) {
        setError(result.error || i18n._(msg`Failed to update company`));
        setIsSubmitting(false);
        return;
      }
      
      router.push(`/dashboard/organization/companies/${company.id}?success=updated`);
    } catch (err: unknown) {
      const errorMessage = err instanceof Error ? err.message : i18n._(msg`Failed to update company`);
  ...
  return (
    <Card>
      <CardHeader title={i18n._(msg`Edit Company`)} />
      ...
      <form onSubmit={handleSubmit} className="p-6 space-y-6">
        ...
        <div className="flex gap-4 pt-4">
          <Button
            ...
            className="flex-1"
          >
            {isSubmitting ? <Trans>Saving Changes...</Trans> : <Trans>Save Changes</Trans>}
          </Button>
          <Button
            type="button"
            variant="outline"
