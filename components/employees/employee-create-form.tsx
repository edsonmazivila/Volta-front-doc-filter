...
export function EmployeeCreateForm({ companyName }: EmployeeCreateFormProps) {
	...
	async function onSubmit(values: FormValues) {
		...
		try {
			...
			if ('errors' in result && result.errors) {
				const errorMsg = result.errors._form?.[0] || Object.values(result.errors).flat().join(', ')
			...
			<form onSubmit={handleSubmit(onSubmit)} className='space-y-6'>
				...
				<div className='bg-card border border-[var(--border)] rounded-lg p-6 space-y-4'>
					<div>
						...
						<p className='text-sm text-muted-foreground'>Create the user account that will be linked to the employee.</p>
					</div>
					<div className='grid grid-cols-1 md:grid-cols-2 gap-4'>
						...
						<div className='flex flex-col gap-1'>
							...
							<Input type='email' {...register('email')} placeholder='employee@company.com' />
				...
				<div className='bg-card border border-[var(--border)] rounded-lg p-6 space-y-4'>
					<div>
						<h3 className='text-lg font-semibold mb-1'>Employment & Organization</h3>
						<p className='text-sm text-muted-foreground'>Link the employee to the company and define their role.</p>
					</div>
					<div className='grid grid-cols-1 md:grid-cols-2 gap-4'>
						<div className='flex flex-col gap-1 md:col-span-2'>
							...
							<p className='text-xs text-muted-foreground'>Company is automatically set based on your account</p>
						</div>
						<div className='flex flex-col gap-1'>
						...
						<div className='flex flex-col gap-1'>
							...
							<Input {...register('employee_number')} placeholder='e.g., EMP001' />
						</div>
						<div className='flex flex-col gap-1'>
						...
						<div className='flex flex-col gap-1'>
							...
							/>
							{errors.employment_type && <span className='text-xs text-destructive'>{errors.employment_type.message}</span>}
						</div>
						<div className='flex flex-col gap-1'>
							<label className='text-sm font-medium'>Hire Date *</label>
							<Input type='date' {...register('hire_date')} />
							{errors.hire_date && <span className='text-xs text-destructive'>{errors.hire_date.message}</span>}
						</div>
					</div>
				</div>
				...
				<div className='bg-card border border-[var(--border)] rounded-lg p-6 space-y-4'>
					<div>
						...
						<p className='text-sm text-muted-foreground'>Optional contact details for the employee.</p>
					</div>
					<div className='grid grid-cols-1 md:grid-cols-2 gap-4'>
						<div className='flex flex-col gap-1'>
							<label className='text-sm font-medium'>Primary Phone</label>
							<Input type='tel' {...register('primary_phone')} placeholder='+1 (555) 000-0000' />
						</div>
						<div className='flex flex-col gap-1'>
							<label className='text-sm font-medium'>Secondary Phone</label>
							<Input type='tel' {...register('secondary_phone')} placeholder='+1 (555) 000-0000' />
						</div>
						<div className='flex flex-col gap-1 md:col-span-2'>
						...
						<div className='flex flex-col gap-1'>
							...
							<Input {...register('postal_code')} placeholder='ZIP/Postal code' />
						</div>
						<div className='flex flex-col gap-1'>
				...
				<div className='bg-card border border-[var(--border)] rounded-lg p-6 space-y-4'>
					<div>
						...
						<p className='text-sm text-muted-foreground'>Who should we contact in an emergency?</p>
					</div>
					<div className='grid grid-cols-1 md:grid-cols-2 gap-4'>
						<div className='flex flex-col gap-1'>
						...
						<div className='flex flex-col gap-1'>
							...
							<Input type='tel' {...register('emergency_contact_phone')} placeholder='+1 (555) 000-0000' />
						</div>
						<div className='flex flex-col gap-1 md:col-span-2'>
							<label className='text-sm font-medium'>Relationship</label>
							<Input {...register('emergency_contact_relationship')} placeholder='e.g., Spouse, Parent, Sibling' />
						</div>
					</div>
				</div>
				...
				<div className='bg-card border border-[var(--border)] rounded-lg p-6 space-y-4'>
					<div>
						...
						<p className='text-sm text-muted-foreground'>Pay structure and hours.</p>
					</div>
					<div className='grid grid-cols-1 md:grid-cols-2 gap-4'>
						<div className='flex flex-col gap-1'>
						...
						<div className='flex flex-col gap-1'>
							...
							<Input type='number' step='0.1' {...register('overtime_rate')} placeholder='e.g., 1.5' />
							<p className='text-xs text-muted-foreground'>Standard is 1.5x for overtime</p>
						</div>
						<div className='flex flex-col gap-1'>
							<label className='text-sm font-medium'>Standard Hours (per week)</label>
							<Input type='number' step='1' {...register('standard_hours')} placeholder='e.g., 40' />
						</div>
					</div>
				</div>
